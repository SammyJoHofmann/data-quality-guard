...
export async function handler(): Promise<void> {
  ...
  const criticalProjects = scores.filter((s: any) => s.overall_score < 40);
  const totalFindings = scores.reduce((sum: number, s: any) => sum + (s.findings_count || 0), 0);

  console.log(`[ScheduledReport] Summary: ${scores.length} projects, ${totalFindings} total findings, ${criticalProjects.length} critical`);

  // Log projects that need attention
